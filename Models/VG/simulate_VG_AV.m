function [S, Sav] = simulate_VG_AV(S0, T, r, N_SIM, N, PARAMS)

% Simulate Variance Gamma prices with Monte Carlo antithetic variables technique
% starting from Gamma subordinators
% S(t) = S0 * exp( rt + X(t) )
% X(t) is a Variance Gamma process in the Risk Neutral measure
...
charexp = @(u) - 1 / K_VG * log( 1 + 0.5 * u^2 * SIGMA^2 * K_VG - 1i * THETA * K_VG * u );
...
for i = 1:N
    ...
    X(:, i+1)   = X(:, i)   + drift * dt + THETA * G(:, i) + SIGMA * sqrt(G(:, i)) .* Z(:, i);
    Xav(:, i+1) = Xav(:, i) + drift * dt + THETA * G(:, i) - SIGMA * sqrt(G(:, i)) .* Z(:, i);
