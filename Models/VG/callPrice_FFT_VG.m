function PRICE = callPrice_FFT_VG(S0, T, r, STRIKE, PARAMS)
...
CharFunc = @(v) exp( T * CharExp_VG(v, SIGMA, THETA, K_VG) );
...
function V = CharExp_VG(u, SIGMA, THETA, K_VG)

% characteristic exponent without drift
V = @(v) - 1 / K_VG * log( 1 + 0.5 * v.^2 * SIGMA.^2 * K_VG - 1i * THETA * K_VG * v );
