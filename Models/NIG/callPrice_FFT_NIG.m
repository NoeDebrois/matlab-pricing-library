function PRICE = callPrice_FFT_NIG(S0, T, r, STRIKE, PARAMS)
...
CharFunc = @(v) exp( T * CharExp_NIG(v, SIGMA, THETA, K_NIG) );
...
function V = CharExp_NIG(u, SIGMA, THETA, K_NIG)

% characteristic exponent without drift
V = @(v) 1 / K_NIG - 1 / K_NIG * sqrt( 1 + v.^2 * SIGMA.^2 * K_NIG - 2i * THETA * K_NIG * v );
