function [PARAMS, ERROR_PRICES, ERROR_VOL] = calibrate_BS(S0, r)

% Calibrate Black-Scholes Model using Carr-Madan algorithm for pricing
...
PARAMS = ...
    lsqnonlin(@(p) abs(arrayfun(@(i) callPrice_FFT_BS(S0, maturities(i), r, strikes(i), p) ...
...
% parameters calibration
PARAMS = ...
    lsqnonlin(@(p) abs(arrayfun(@(i) blsimpv(S0, strikes(i), r, maturities(i), callPrice_FFT_BS(S0, maturities(i), r, strikes(i), p)) ...
                                      - blsimpv(S0, strikes(i), r, maturities(i), mkt_prices(i)), [1:length(strikes)]')), x0, LB, UB, options);

% └------------------------------------------------------------------------------------------------------------------------┘

% compute the calibrated model prices
prices = arrayfun(@(i) callPrice_FFT_BS(S0, maturities(i), r, strikes(i), PARAMS), [1:length(strikes)]');
...
legend('Market prices', 'Calibrated Black & Scholes prices')
xlabel('Strike : K')
ylabel('Prices')
...
legend('Market Implied Volatility', 'Black & Scholes Implied Volatility')
xlabel('Strike : K')
