function [PARAMS, ERROR_PRICES, ERROR_VOL] = calibrate_Bates(S0, r)

% Calibrate Bates Model using Carr-Madan algorithm for pricing

% theta  = PARAMS(1) - vol-of-vol
% xi     = PARAMS(2) - mean reversion speed
% rho    = PARAMS(3) - correlation
% eta    = PARAMS(4) - mean reversion value
% V0     = PARAMS(5) - starting point for the volatility
% kappa  = PARAMS(6) - jump size
...
LB = [eps; eps;    -1;  eps; eps; eps; eps; eps];
x0 = [0.2; 0.5; -0.25; 0.15; 0.2; 0.5; 0.2;  30]; % theta, xi, rho, eta, V0, kappa, delta, lambda
UB = [  1;   3;     1;    1;   1;   4;   1; 100];
...
PARAMS = ...
    lsqnonlin(@(p) abs(arrayfun(@(i) callPrice_FFT_Bates(S0, maturities(i), r, strikes(i), p) ...
...
% parameters calibration
PARAMS = ...
    lsqnonlin(@(p) abs(arrayfun(@(i) blsimpv(S0, strikes(i), r, maturities(i), callPrice_FFT_Bates(S0, maturities(i), r, strikes(i), p)) ...
                                      - blsimpv(S0, strikes(i), r, maturities(i), mkt_prices(i)), [1:length(strikes)]')), x0, LB, UB, options);

% └------------------------------------------------------------------------------------------------------------------------┘

% compute the calibrated model prices
prices = arrayfun(@(i) callPrice_FFT_Bates(S0, maturities(i), r, strikes(i), PARAMS), [1:length(strikes)]');
...
legend('Market prices', 'Calibrated Bates prices')
xlabel('Strike : K')
ylabel('Prices')
...
legend('Market Implied Volatility', 'Bates Implied Volatility')
xlabel('Strike : K')
