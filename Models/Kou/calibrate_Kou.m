function [PARAMS, ERROR_PRICES, ERROR_VOL] = calibrate_Kou(S0, r)

% Calibrate Kou Model using Carr-Madan algorithm for pricing
...
LB = [eps; eps;  0;  0;   0];
x0 = [0.2;  30;  2;  2; 0.5]; % sigma, lambda, lambda minus, lambda plus, p
UB = [  1; 100; 20; 20;   1];
...
PARAMS = ...
    lsqnonlin(@(p) abs(arrayfun(@(i) callPrice_FFT_Kou(S0, maturities(i), r, strikes(i), p) ...
...
% parameters calibration
PARAMS = ...
    lsqnonlin(@(p) abs(arrayfun(@(i) blsimpv(S0, strikes(i), r, maturities(i), callPrice_FFT_Kou(S0, maturities(i), r, strikes(i), p)) ...
                                      - blsimpv(S0, strikes(i), r, maturities(i), mkt_prices(i)), [1:length(strikes)]')), x0, LB, UB, options);

% └------------------------------------------------------------------------------------------------------------------------┘

% compute the calibrated model prices
prices = arrayfun(@(i) callPrice_FFT_Kou(S0, maturities(i), r, strikes(i), PARAMS), [1:length(strikes)]');
...
legend('Market prices', 'Calibrated Kou prices')
xlabel('Strike : K')
ylabel('Prices')
...
legend('Market Implied Volatility', 'Kou Implied Volatility')
xlabel('Strike : K')
