function PRICE = callPrice_FFT_ExtVG(S0, T, r, STRIKE, PARAMS)

% Price of a plain vanilla call exploiting the Carr-Madan algorithm
% Model: Extended Variance Gamma
...
CharFunc = @(v) exp( T * CharExp_ExtVG(v, SIGMA, THETA, K_VG, SIGMA_BAR) );
...
function V = CharExp_ExtVG(u, SIGMA, THETA, K_VG, SIGMA_BAR)

% characteristic exponent without drift
V = @(v) - SIGMA_BAR^2/2 * v.^2 - 1 / K_VG * log( 1 + 0.5 * v.^2 * SIGMA.^2 * K_VG - 1i * THETA * K_VG * v );
