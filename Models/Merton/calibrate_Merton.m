function [PARAMS, ERROR_PRICES, ERROR_VOL] = calibrate_Merton(S0, r)

% Calibrate Merton Model using Carr-Madan algorithm for pricing
...
LB = [eps; eps; -3; eps];
x0 = [0.2;  50;  0; 0.2]; % sigma, lambda, mu, delta
UB = [  1; 200;  3;   1];
...
PARAMS = ...
    lsqnonlin(@(p) abs(arrayfun(@(i) callPrice_FFT_Merton(S0, maturities(i), r, strikes(i), p) ...
...
% parameters calibration
PARAMS = ...
    lsqnonlin(@(p) abs(arrayfun(@(i) blsimpv(S0, strikes(i), r, maturities(i), callPrice_FFT_Merton(S0, maturities(i), r, strikes(i), p)) ...
                                      - blsimpv(S0, strikes(i), r, maturities(i), mkt_prices(i)), [1:length(strikes)]')), x0, LB, UB, options);

% └------------------------------------------------------------------------------------------------------------------------┘

% compute the calibrated model prices
prices = arrayfun(@(i) callPrice_FFT_Merton(S0, maturities(i), r, strikes(i), PARAMS), [1:length(strikes)]');
...
legend('Market prices', 'Calibrated Merton prices')
xlabel('Strike : K')
ylabel('Prices')
...
legend('Market Implied Volatility', 'Merton Implied Volatility')
xlabel('Strike : K')
