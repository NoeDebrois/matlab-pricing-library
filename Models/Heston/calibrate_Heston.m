function [PARAMS, ERROR_PRICES, ERROR_VOL] = calibrate_Heston(S0, r)

% Calibrate Heston Model using Carr-Madan algorithm for pricing

% epsilon = PARAMS(1) - vol-of-vol
% k       = PARAMS(2) - mean reversion speed
% rho     = PARAMS(3) - correlation
% theta   = PARAMS(4) - mean reversion value
...
LB = [eps; eps;    -1;  eps; eps];
x0 = [0.2; 0.5; -0.25; 0.15; 0.2]; % epsilon, k, rho, theta, V0
UB = [  1;   3;     1;    1;   1];
...
PARAMS = ...
    lsqnonlin(@(p) abs(arrayfun(@(i) callPrice_FFT_Heston(S0, maturities(i), r, strikes(i), p) ...
...
% parameters calibration
PARAMS = ...
    lsqnonlin(@(p) abs(arrayfun(@(i) blsimpv(S0, strikes(i), r, maturities(i), callPrice_FFT_Heston(S0, maturities(i), r, strikes(i), p)) ...
                                      - blsimpv(S0, strikes(i), r, maturities(i), mkt_prices(i)), [1:length(strikes)]')), x0, LB, UB, options);

% └------------------------------------------------------------------------------------------------------------------------┘

% compute the calibrated model prices
prices = arrayfun(@(i) callPrice_FFT_Heston(S0, maturities(i), r, strikes(i), PARAMS), [1:length(strikes)]');
