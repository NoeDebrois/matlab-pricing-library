...
% Option considered: Fixed Strike Lookback Put on Maximum
% Payoff: { K - M(T) }⁺ where M(T) = max_t ∈ [0, T] ( S(t) )
% Note: S0 must be smaller than K to have a non zero price of the option
...
DiscPayoff = exp( - r * T ) * max(K - max(S, [], 2), 0);
...
% - Compute the discounted payoff
DiscPayoff   = exp( - r * T ) * max(K - max(S, [], 2), 0);
DiscPayoffav = exp( - r * T ) * max(K - max(Sav, [], 2), 0);
