...
% Option considered: Float Strike Lookback Call on Minimum
% Payoff: { S(T) - m(T) }⁺ where m(T) = min_t ∈ [0, T] ( S(t) )
% Note: a Put on minimum would make no sense: { m(T) - S(T) }⁺ = 0
...
DiscPayoff = exp( - r * T ) * max(S(:, end) - min(S, [], 2), 0);
...
% - Compute the discounted payoff
DiscPayoff   = exp( - r * T ) * max(S(:, end) - min(S, [], 2), 0);
DiscPayoffav = exp( - r * T ) * max(Sav(:, end) - min(Sav, [], 2), 0);
