...
% Option considered: Floating Strike Asian Put Option
% Payoff: { 1/N * sum_i=1->N ( S(i) ) - S(T) }⁺
...
% S(T) - 1/N * sum_i=1->N ( S(i) )
DiscPayoff = exp( - r * T ) * max(mean(S, 2) - S(:,end), 0);
...
% - Compute the discounted payoff
DiscPayoff   = exp( - r * T ) * max(mean(S,2) - S(:,end), 0);
DiscPayoffav = exp( - r * T ) * max(mean(Sav, 2) - Sav(:,end), 0);
...
% > 3.1. Choose a control variable: f = mean(S, 2) - S(T)

% - expected value of f
Ef = S0/(N+1) * sum( exp( r * (0:N) * dt ) ) - S0 * exp( r * T );
