...
% Option considered: Fixed Strike Asian Put Option
% Technique: Monte Carlo, Monte Carlo AV , Monte Carlo CV
% Payoff: { K - 1/N * sum_i=1->N ( S(i) ) }⁺
...
DiscPayoff = exp( - r * T ) * max(K - mean(S, 2), 0);
...
% - Compute the discounted payoff
DiscPayoff   = exp( - r * T ) * max(K - mean(S, 2), 0);
DiscPayoffav = exp( - r * T ) * max(K - mean(Sav, 2), 0);
...
% > 3.1. Choose a control variable: f = K - mean(S, 2) or mean(S, 2)

% - expected value of f
Ef = K - S0/(N+1) * sum( exp( r * (0:N) * dt ) );
% alternative:
%Ef = S0/(N+1) * sum( exp( r * (0:N) * dt ) );
