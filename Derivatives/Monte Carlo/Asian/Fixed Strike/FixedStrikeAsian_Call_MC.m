...
% Option considered: Fixed Strike Asian Call Option
% Payoff: { 1/N * sum_i=1->N ( S(i) ) - K }⁺
...
% - Compute the discounted payoff
DiscPayoff   = exp( - r * T ) * max(mean(S, 2) - K, 0);
DiscPayoffav = exp( - r * T ) * max(mean(Sav, 2) - K, 0);
...
% > 3.1. Choose a control variable: f = mean(S, 2) - K or mean(S, 2)

% - expected value of f
Ef = S0/(N+1) * sum( exp( r * (0:N) * dt ) ) - K;
% alternative:
